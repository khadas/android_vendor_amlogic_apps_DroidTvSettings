...
public class DisplayPositionActivity extends DialogActivity implements ActionAdapter.Listener {
	...
	private void updateMainScreen() {
		mContentFragment.setDescriptionText(getString(R.string.device_position_description) + " "
				+ mDisplayPositionManager.getCurrentRateValue() + "%");
	}

	private ContentFragment createMainMenuContentFragment() {
		return ContentFragment.newInstance(getString(R.string.device_position),
				getString(R.string.device_display), getString(R.string.device_position_description) + " "
						+ mDisplayPositionManager.getInitialRateValue() + "%",
				R.drawable.ic_settings_overscan, getResources().getColor(R.color.icon_background));
	}
	...
	private ArrayList<Action> getMainActions() {
		...
		actions.add(new Action.Builder().key(zoom_in)
				.title("               " + getString(R.string.device_position_zoomin)).build());

		actions.add(new Action.Builder().key(zoom_out)
				.title("               " + getString(R.string.device_position_zoomout)).build());
