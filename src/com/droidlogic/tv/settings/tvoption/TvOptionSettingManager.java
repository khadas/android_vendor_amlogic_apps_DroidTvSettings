...
public class TvOptionSettingManager {
    ...
    public int getSwitchChannelStatus () {
        int ret = 0;
        try{
            Method method = TvControlManager.class.getMethod("getBlackoutEnalbe()");
            ret = (int)method.invoke(mTvControlManager);
    ...
    private void setDefAudioStreamVolume() {
        ...
        int streamMaxVolume = audioManager.getStreamMaxVolume(3/*AudioSystem.STREAM_MUSIC*/);
        int defaultVolume = maxVolume == streamMaxVolume ? (maxVolume * 3) / 10 : (streamMaxVolume * 3) / 4;
        audioManager.setStreamVolume(3/*AudioSystem.STREAM_MUSIC*/, defaultVolume, 0);
    }
